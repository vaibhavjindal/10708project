...
import os
os.makedirs(f'{PATH}/perf_lists_cifar_vae', exist_ok=True)
...
vae = vae.from_pretrained(f'cifar10-resnet18').to(device)
...
def get_new_images(X):
    vae.eval()
    X_hat = vae(normalize(X.to(device)))
...
def save_list(input_list, name):
    with open(f'{PATH}/perf_lists_cifar_vae/' + name, 'w') as f:
