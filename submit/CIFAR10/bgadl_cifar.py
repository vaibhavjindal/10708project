# -*- coding: utf-8 -*-
"""
This code is a modified version of the code present here:
https://github.com/toantm/BGADL
"""
...
train_loader_all = torch.utils.data.DataLoader(
    datasets.CIFAR10('../data2', train=True, download=True,
...
test_loader = torch.utils.data.DataLoader(
    datasets.CIFAR10('../data2', train=False, transform=transforms.Compose([
                       transforms.ToTensor(),
                       transforms.Resize(size=(28,28))
                   ])),
    batch_size=batch_size, shuffle=True, **kwargs)
...
def prepare_data():
    train_data_all = torch.tensor(train_loader_all.dataset.data)
    train_target_all = torch.tensor(train_loader_all.dataset.targets)
    print(type(train_data_all), train_data_all.shape, type(train_target_all), train_target_all.shape)
    ...
    test_data = torch.tensor(test_loader.dataset.data)
    test_target = torch.tensor(test_loader.dataset.targets)

    print(type(test_data), test_data.shape, type(test_target), test_target.shape)
    ...
    print(type(train_data_pool), train_data_pool.shape,type(train_target_pool), train_target_pool.shape)
    
    train_data_pool = torch.permute(train_data_pool, (0,3,1,2))
    train_data_all = torch.permute(train_data_all, (0,3,1,2))
    test_data = torch.permute(test_data, (0,3,1,2))

    # train_data_pool.unsqueeze_(1)
    # train_data_all.unsqueeze_(1)
    # test_data.unsqueeze_(1)

    print(train_data_pool.shape, train_data_all.shape, test_data.shape)
...
pool_data = transform(pool_data)
test_data = transform(test_data)


print(train_data.shape,train_target.shape,pool_data.shape)
print(pool_target.shape, test_data.shape, test_target.shape)
...
def initialize_train_set():
    ...
    train = data_utils.TensorDataset(transform(train_data), train_target)
...
def test(epoch):
    ...
    test = data_utils.TensorDataset(transform(test_data), test_target)
...
class VAEACGAN(object):
    def __init__(self):
        ...
        self.data_X = transform(train_data)
        ...
        self.X_test = transform(test_data)
    ...
    def train(self):
        ...
        self.save()
        generate_animation(self.result_dir + '/'  + self.model_name + '/' + self.model_name,
                                 self.epoch)
